...
local function printAVP(array)
    for index, avp in pairs(array) do
        print(string.format(" AVP %s = %s " , avp.name, avp.value))
    end
end
...
do
    ...
    local subField = Field.new("diameter.Subscription-Id-Data")
    local sutField = Field.new("diameter.Subscription-Id-Type")
    local rgField  = Field.new("diameter.Rating-Group")
    ...
    local function init_listener()
        ...
        -- local totalOctets = {}
        -- local filter = 'diameter.Auth-Application-Id == 4'
        local filter = nil
        ...
        tap = Listener.new("frame", filter, true)
        
        -- this function will be called once for each packet
        function tap.packet(pinfo, tvb, tapinfo)
            local reqFields = { reqField() }
            local sidFields = { sidField() }
            local subFields = { subField() }
            local sutFields = { sutField() }
            local rgFields  = { rgField()  }
            local rrFields  = { rrField()  }
            local toFields  = { toField()  }
            local rcFields  = { rcField()  }

            print(" Tapdata type: " .. type(tapinfo))
            ...
            then
                ...
                for index, _ in pairs(sidFields) do
                    ...
                    then
                        ...
                        for index, _ in pairs(subFields) do
                            -- Check if Subscription-Id-Type == END_USER_E164 (0)
                            if( sutFields[index].value == 0)
                            then
                                print(string.format("   %s - %s", subFields[index], sutFields[index]))
                                sessions[sid] = tostring(subFields[index])
                            end
                        end
                        ...
                        if( sessions[sid] ~= nil)
                        then
                            --  Dump all intersting info
                            printAVP(rgFields)
                            printAVP(rrFields)
                            printAVP(toFields)
                            printAVP(rcFields)

                            print(string.format(" Going to remove sessions[%s] = %s ", sid, sessions[sid]))
                            table.remove(sessions, sid)
                        end
                        print(" Number of sessions (2) " .. #sessions)
                    end 
    ...
    if gui_enabled() == true then
        ...
        register_menu("Statistics/Diameter extract DCCA", init_listener, MENU_TOOLS_UNSORTED)
